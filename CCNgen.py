...
def replace_and_visualize_confusion_matrix(data, target_class, replacement_class, num_replacements):
    ...
    if num_replacements==0:
        ...
        # 將選中的索引對應的值替換為替代類別
        indices_to_replace = target_indices[:num_replacements]
...
def plot_multiple_confusion_matrices(confusion_matrices, titles,target,replace):
    ...
    for i, ax in enumerate(axes):
        sns.heatmap(confusion_matrices[i], annot=True, fmt='d', cmap='Blues', ax=ax, xticklabels= xticklabels, yticklabels=yticklabels)
